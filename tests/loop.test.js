const { test, expect } = require("@playwright/test");

const testCases = [
  {
    id: 1,
    name: "Test Case 1",
    leftNav: "Cross-functional project plan, Project",
    column: "To do",
    card_title: "Draft project brief",
  },
  {
    id: 2,
    name: "Test Case 2",
    leftNav: "Cross-functional project plan, Project",
    column: "To do",
    card_title: "Schedule kickoff meeting",
  },
  {
    id: 3,
    name: "Test Case 3",
    leftNav: "Cross-functional project plan, Project",
    column: "To do",
    card_title: "Share timeline with teammates",
  },
  {
    id: 4,
    name: "Test Case 4",
    leftNav: "Work Requests",
    column: "New Requests",
    card_title: "[Example] Laptop setup for new hire",
  },
  {
    id: 5,
    name: "Test Case 5",
    leftNav: "Work Requests",
    column: "In Progress",
    card_title: "[Example] Password not working",
  },
  {
    id: 6,
    name: "Test Case 6",
    leftNav: "Work Requests",
    column: "Completed",
    card_title: "[Example] New keycard for Daniela V",
  },
];

test.describe("Asana Data-Driven Tests", () => {
  testCases.forEach((data) => {
    test(data.name, async ({ page }) => {
      await test.step("Login to Asana", async () => {
        await page.goto("https://app.asana.com/-/login");
        await page.getByLabel("Email address").click();
        await page
          .getByLabel("Email address")
          .fill("ben+pose@workwithloop.com");
        await page
          .getByRole("button", { name: "Continue", exact: true })
          .click();
        await page.getByLabel("Password", { exact: true }).fill("Password123");
        await page.getByRole("button", { name: "Log in" }).click();
      });

      await test.step("Navigate to the project page", async () => {
        await page.getByRole("button", { name: "Projects" }).click();
        await page.getByText("Browse projects").click();
        await page.getByRole("link", { name: `${data.leftNav}` }).click();
      });

      await test.step("Verify the card is within the right column", async () => {
        const column = await page.getByRole("heading", { name: data.column });
        await expect(column).toBeVisible();
        const card = await page.getByText(data.card_title, { exact: true });
        await expect(card).toBeVisible();
      });
    });
  });
});

// navigating to the project page rather than clicking invdivdual projects to view all projects
//ensure navigating to project page is highlighing correct data.leftNav
// its navigating to projects and somehiow defaulting to cross functional string and not working for the rest
